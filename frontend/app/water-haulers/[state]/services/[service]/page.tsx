...
import { STATE_NAMES, fromStateSlug } from "@/lib/location";
import haulersFlatData from "@/data/haulers-flat.json";

type PageProps = { params: Promise<{ state: string; service: string }> };
...
const SERVICE_CONTENT: Record<string, { intro: string; faqs: Array<{ question: string; answer: string }>; ctaButton: string }> = {
  "pool-fill": {
    intro: "Need to fill or top off a pool? HaulAgua connects you with reliable water delivery services in [State]. From new pool fills to seasonal maintenance, find vetted professionals who deliver potable water quickly and affordably.",
    faqs: [
...
function getHaulersByStateAndService(
  state: string,
  service: string
...
export async function generateStaticParams() {
  ...
  for (const h of haulersFlatData as typeof haulersFlatData) {
    if (h.isActive !== false) states.add(h.state.toLowerCase());
  }

  const result: { state: string; service: string }[] = [];
  for (const state of states) {
    for (const service of services) {
      result.push({ state, service });
    }
  }
  return result;
}
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const title = `${serviceTitle} in ${stateName} | HaulAgua`;
  const description = `Find reliable water haulers for ${serviceTitle.toLowerCase()} services in ${stateName}. Vetted professionals, fast delivery, competitive rates.`;
  ...
  return {
    ...
    alternates: {
      canonical: `https://www.haulagua.com/water-haulers/${state}/services/${service}`,
    },
    openGraph: {
      title,
      description,
      url: `https://www.haulagua.com/water-haulers/${state}/services/${service}`,
      siteName: "HaulAgua",
      type: "website",
    },
  };
...
export default async function StateServicePage({ params }: PageProps) {
  ...
  const haulers = getHaulersByStateAndService(state, service);
  const serviceTitle = {
    "pool-fill": "Pool Fill",
    construction: "Construction & Dust Control",
    potable: "Potable Water Delivery",
          ...
          <div className={INNER}>
            ...
            <div className="max-w-3xl">
              <h1 className="font-serif text-3xl md:text-4xl lg:text-5xl font-bold text-white">
                {serviceTitle} in {stateName}
              </h1>
              <p className="mt-4 text-lg text-white/85 max-w-xl">
                {content.intro.replace("[State]", stateName)}
              </p>
            ...
            {haulers.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-24 text-center gap-4">
                ...
                <p className="text-muted-foreground max-w-sm">
                  We don't have {serviceTitle.toLowerCase()} services listed in {stateName} yet.{" "}
              ...
              <>
                <h2 className="font-serif text-2xl font-semibold mb-6" style={{ color: "#333333" }}>
                  {serviceTitle} Providers in {stateName}
                </h2>
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
                  {haulers.map((hauler) => (
                    <HaulerCard key={hauler.slug} hauler={hauler as any} refPath={`/water-haulers/${state}/services/${service}`} />
                  ))}
                </div>
              </>
            )}
            ...
            <section className="py-8 md:py-8" style={{ background: "linear-gradient(to bottom, #F8FBFF, #F0F6FC)" }}>
              <div className={INNER}>
                <div className="rounded-xl bg-white border border-[#005A9C]/20 p-8 text-center shadow-sm">
                  ...
                  <p className="text-muted-foreground mb-6 max-w-md mx-auto">Contact the haulers above or browse more in {stateName}.</p>
                  <Link
                    href={`/water-haulers/${state}`}
