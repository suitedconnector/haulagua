...
const TX_FAQS = [
  { q: "How much does bulk water delivery cost in Texas?", a: "Bulk water delivery in Texas typically costs between $150 and $500 per load, depending on the hauler, distance, water type, and volume. Potable water generally costs more than non-potable water. Most haulers have a minimum fee ranging from $150 to $300. Request quotes from multiple haulers in your area for the best rate." },
  { q: "How many gallons does a water hauler truck hold in Texas?", a: "Most water hauler trucks in Texas carry between 2,000 and 6,000 gallons per load. Smaller medium-duty trucks typically hold 2,000–3,000 gallons, while full-size tanker trucks carry 4,000–6,000 gallons or more. For large jobs like filling a pool or topping off a cistern, haulers may make multiple trips." },
  { q: "How long does it take to fill a pool with a water truck in Texas?", a: "A standard residential pool (10,000–20,000 gallons) typically requires 2–4 truckloads and can be filled in a few hours to one day, depending on truck size and travel time. Most Texas haulers can complete a pool fill in a single day with proper scheduling." },
  { q: "Do Texas water haulers need a license for potable water delivery?", a: "Yes. Haulers delivering potable (drinking) water in Texas are required to be licensed by the Texas Commission on Environmental Quality (TCEQ). Always confirm your hauler holds a valid TCEQ potable water carrier license before accepting drinking water delivery." },
  { q: "What areas of Texas have the most water haulers?", a: "The highest concentration of bulk water haulers in Texas is found in Central Texas (Austin metro, Hill Country), West Texas (Permian Basin), and the DFW metroplex. Rural counties in East, South, and Far West Texas tend to have fewer haulers, so lead times may be longer in those areas." },
  { q: "Can I get emergency water delivery in Texas?", a: "Yes. Many Texas water haulers offer emergency or same-day delivery for well failures, drought conditions, fire suppression needs, and disaster response. Search for haulers in your city and call directly — most can advise on availability within minutes." },
  { q: "What is the difference between potable and non-potable water delivery?", a: "Potable water is safe for human consumption — drinking, cooking, and bathing. Non-potable water is not safe to drink and is used for pool fills (chlorinated separately), dust control, irrigation, and construction. Potable delivery requires a licensed carrier and a food-grade tanker. Always specify which type you need when requesting a quote." },
];
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return {
    title, description,
    openGraph: { title, description, url: `https://haulagua.com/water-haulers/${state}`, siteName: "HaulAgua", type: "website" },
  };
}
...
type CityWithPhoto = { city: string; slug: string; count: number; photo: string | null };
...
export default async function StatePage({ params }: PageProps) {
  ...
  const citiesWithPhotos: CityWithPhoto[] = cities.map(({ city, slug, count }) => ({
    city, slug, count, photo: cache[`${city}|${abbr}`] ?? null,
  }));
  const hasCities = citiesWithPhotos.length > 0;

  const faqSchema = isTX ? {
    "@context": "https://schema.org", "@type": "FAQPage",
    mainEntity: TX_FAQS.map(({ q, a }) => ({ "@type": "Question", name: q, acceptedAnswer: { "@type": "Answer", text: a } })),
  } : null;

  const itemListSchema = haulers.length > 0 ? {
    "@context": "https://schema.org", "@type": "ItemList",
    name: `Bulk Water Haulers in ${stateName}`,
    itemListElement: haulers.map((h, i) => ({ "@type": "ListItem", position: i + 1, name: h.attributes.name, url: `https://haulagua.com/haulers/${h.attributes.slug}` })),
  } : null;
  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      <main className="flex-1">
        ...
        <section className="py-12 md:py-16" style={{ background: "linear-gradient(to bottom, #005A9C, #2A7FC1)" }}>
          <div className={INNER}>
            <Link href="/water-haulers" className="inline-flex items-center gap-1.5 text-sm font-medium mb-6 text-white/80 hover:text-white transition-colors">
            ...
            {isTX ? (
              <div className="mt-4 space-y-3 text-lg text-white/85 max-w-2xl">
        ...
        <div style={{ backgroundColor: "rgb(200, 220, 240)" }}>
        ...
        {/* ── Cities — white, cards provide the color ── */}
        {hasCities && (
          <>
            <section className="py-12 md:py-16 bg-white">
              <div className={INNER}>
                <h2 className="font-serif text-xl font-semibold mb-6" style={{ color: "#333333" }}>
                  Browse by City
                </h2>
                    ...
                    >
                      <div className="relative h-40 w-full overflow-hidden">
                        {photo ? (
                          ...
                          <div className="w-full h-full" style={{ backgroundColor: "#005A9C" }} />
                        )}
                        ...
                        <div className="absolute inset-0 flex items-end justify-center pointer-events-none" style={{ paddingBottom: "16px" }}>
                          <div style={{ display: "flex", flexDirection: "column", alignItems: "center" }}>
                            <div style={{ backgroundColor: "#2d6e2d", border: "4px solid white", borderRadius: "6px", padding: "8px 18px", textAlign: "center", boxShadow: "0 4px 16px rgba(0,0,0,0.5)", minWidth: "160px", maxWidth: "88%" }}>
                              <p style={{ color: "white", fontSize: "9px", fontWeight: 700, letterSpacing: "0.18em", textTransform: "uppercase", marginBottom: "2px" }}>Welcome to</p>
                              <p style={{ color: "white", fontSize: "20px", fontWeight: 800, lineHeight: 1.1, fontFamily: "Georgia, serif" }}>{city}</p>
                              <p style={{ color: "white", fontSize: "9px", fontWeight: 700, letterSpacing: "0.18em", textTransform: "uppercase", marginTop: "2px" }}>{stateName}</p>
                            </div>
                      ...
                      <div className="px-4 py-3 flex items-center gap-2" style={{ backgroundColor: "#0461AA" }}>
                        <p className="text-sm font-bold text-white">{count} hauler{count !== 1 ? "s" : ""}</p>
        ...
        {/* ── Regional Coverage — lighter gradient variant, TX only ── */}
        {isTX && (
          <>
            <section className="py-12 md:py-16" style={{ background: "linear-gradient(to bottom, #D6E8F7, #F8FBFF)" }}>
              <div className={INNER}>
                <h2 className="font-serif text-2xl font-semibold mb-6" style={{ color: "#333333" }}>
            ...
            {haulers.length === 0 ? (
              <div className="flex flex-col items-center justify-center py-24 text-center gap-4">
                ...
                <p className="text-muted-foreground max-w-sm">
                  ...
                  <Link href="/for-haulers" className="underline" style={{ color: "#005A9C" }}>list your business free</Link>.
                </p>
              </div>
            ) : (
              <>
                ...
                <div className="space-y-4">
                  {TX_FAQS.map(({ q, a }) => (
                    <div key={q} className="rounded-xl bg-white p-6 shadow-sm">
                      <h3 className="font-semibold text-base mb-2" style={{ color: "#333333" }}>{q}</h3>
