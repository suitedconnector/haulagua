...
import { STATE_NAMES, fromStateSlug, toCitySlug } from "@/lib/location";
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return {
    ...
    alternates: {
      canonical: `https://www.haulagua.com/water-haulers/${state}/${service}`,
    },
    openGraph: {
      title,
      description,
      url: `https://www.haulagua.com/water-haulers/${state}/${service}`,
      siteName: "HaulAgua",
      type: "website",
    },
  };
          ...
          <div className={INNER}>
            ...
            <div className="max-w-3xl">
              ...
              <p className="mt-4 text-lg text-white/85 max-w-xl">
                {content.intro.replace("[State]", stateName).replace("[City]", stateName)}
              </p>
            ...
            {haulers.length === 0 ? (
              ...
              <>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
                  {haulers.map((hauler) => (
                    <HaulerCard key={hauler.slug} hauler={hauler as any} refPath={`/water-haulers/${state}/${service}`} />
                  ))}
                </div>
              </>
            )}
            ...
            <section className="py-8 md:py-8" style={{ background: "linear-gradient(to bottom, #F8FBFF, #F0F6FC)" }}>
              <div className={INNER}>
                <div className="rounded-xl bg-white border border-[#005A9C]/20 p-8 text-center shadow-sm">
                  ...
                  <p className="text-muted-foreground mb-6 max-w-md mx-auto">Contact the haulers above or browse all options in {stateName}.</p>
                  <Link
                    href={`/water-haulers/${state}`}
